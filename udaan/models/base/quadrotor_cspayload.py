...
class QuadrotorCSPayload(BaseModel):
    ...
    def _init_default_controllers(self):
        ...
        # self._prop_controller = control.quadrotor.QuadPropForceController(mass=self.mass, inertia=self.inertia)
        self._pos_controller = control.quadrotor.PositionPDController(mass=self.qrotor_mass)
        return
    ...
    #     while self._mjMdl.data.time < tf:
    ...
    #     print("Took (%.4f)s for simulating (%.4f)s" % (time_taken, tf))
    #     return
